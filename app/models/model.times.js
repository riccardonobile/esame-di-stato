...
let getAll = function(callback) {
    let query = fs.readFileSync(__dirname + '/queries/times/query.getAll.sql').toString();
...
let insert = function(runnerId, raceId, time, callback) {
    let query = fs.readFileSync(__dirname + '/queries/times/query.insert.sql').toString();
